...
master=master %>% select(flag,timestamp)
library(lubridate)
...
test=master %>% select(gap_hours, gap_km,lat,lon) %>% as.matrix() %>% scale()
test=master %>% select(gap_hours, gap_km) %>% as.matrix() %>% scale()
heatmap(test,Colv = F,scale='row')

master=a %>% filter(on_fishing_list_best=="True") %>% filter(type=="AIS.1"|type=="AIS.2"|type=="AIS.3") %>% filter(distance_from_shore_m>4000&next_distance_from_shore_m>4000) %>% mutate(seg_id=substr(seg_id,11,20)) %>% .[complete.cases(.),]
test=master %>% select(flag,vessel_class,gap_hours,gap_km,lat,lon) %>% group_by(flag,vessel_class) %>% summarise(hours=sum(gap_hours),km=sum(gap_km),lat=mean(lat),lon=mean(lon))
test2=test %>% mutate(flag_class=paste0(flag,"_",vessel_class)) %>% select(-c(flag,vessel_class)) %>% as.data.frame()
rownames(test2)=test2$flag_class
test3=test2[,2:ncol(test2)-1] %>% select(-c(flag)) %>% as.matrix() %>% scale()
...
master=a %>% filter(on_fishing_list_best=="True") %>% mutate(seg_id=substr(seg_id,11,20)) %>% .[complete.cases(.),]
test=master %>% select(gap_hours,gap_km,lat,lon,distance_from_shore_m,next_lat,next_lon,next_distance_from_shore_m,vessel_class,flag)%>% group_by(flag,vessel_class) %>% 
...
test2=test %>% mutate(flag_class=paste0(flag,"_",vessel_class))%>% as.data.frame() %>% select(-c(flag,vessel_class)) 
rownames(test2)=test2$flag_class
test3=test2[4:nrow(test2)-1,] %>% select(-c(flag_class)) %>% as.matrix() %>% scale() %>% .[2:nrow(.),]
...
master=a %>% filter(on_fishing_list_best=="True") %>% mutate(seg_id=substr(seg_id,11,20)) %>% .[complete.cases(.),]
test=master %>% select(gap_hours,gap_km,distance_from_shore_m,next_distance_from_shore_m,vessel_class,flag)%>% group_by(flag,vessel_class) %>% 
...
master=a %>% filter(on_fishing_list_best=="True") %>% mutate(seg_id=substr(seg_id,11,20)) %>% .[complete.cases(.),]
test=master %>% select(gap_hours,gap_km,lat,lon,distance_from_shore_m,next_lat,next_lon,next_distance_from_shore_m,vessel_class,flag,timestamp) %>% mutate(m=month(timestamp))%>% group_by(flag,vessel_class,m) %>% 
  summarise(hours=mean(gap_hours),km=mean(gap_km),lat=mean(lat),lon=mean(lon),distance_from_shore_m=mean(distance_from_shore_m),next_lat=mean(next_lat),next_lon=mean(next_lon),next_distance_from_shore_m=mean(next_distance_from_shore_m))
test2=test %>% mutate(flag_class=paste0(flag,"_",vessel_class,"_",m))%>% as.data.frame() %>% select(-c(flag,vessel_class,m)) 
rownames(test2)=test2$flag_class
test3=test2[23:nrow(test2)-1,] %>% select(-c(flag_class)) %>% as.matrix() %>% scale()
heatmap(test3,Colv = F,scale='none')
